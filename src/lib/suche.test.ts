...
describe('undo: a step takes back exactly what it added', () => {
  ...
  it('a step names the records its inverse writes, so a merge that left them alone keeps it (#234)', () => {
    ...
    const d0 = addBereich(a, { name: 'Trakt 3' }, w.cx()).doc
    const bid = idOf(d0, 'Trakt 3')
  ...
  it('the rework\'s steps name their records too: the one-step «＋ Vermisst» at a new place, the tick, a pin put on / moved / taken off', () => {
    const w = world()
    const a = addPerson(emptySuche(), { name: 'Muster Tim', wo: 'Keller', point: { coord: [7.6, 47.5] } }, w.cx())
    const kid = a.doc.bereiche[0].id
    expect(patchTouches(diffSuche(emptySuche(), a.doc)).sort()).toEqual([sucheRecordKey('bereiche', kid), sucheRecordKey('personen', a.person.id)].sort())
    const ticked = toggleAbgesucht(a.doc, kid, w.cx()).doc
    expect(patchTouches(diffSuche(a.doc, ticked))).toEqual([sucheRecordKey('bereiche', kid)])
    const moved = setPlacePoint(ticked, 'bereiche', kid, { planId: 'gebaeude', x: 0.5, y: 0.5, floor: 1 }, w.cx()).doc
    expect(patchTouches(diffSuche(ticked, moved))).toEqual([sucheRecordKey('bereiche', kid)])
    const gone = setPlacePoint(moved, 'bereiche', kid, null, w.cx()).doc
    expect(patchTouches(diffSuche(moved, gone))).toEqual([sucheRecordKey('bereiche', kid)])
  })
})
