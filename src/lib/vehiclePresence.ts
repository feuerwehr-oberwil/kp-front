 ...
 *
 * `fixes` is keyed by Traccar device id → ISO fix time, read straight off the positions feed —
...
/** How long after the shift the Meldeleiste still shows it. A device opened an hour later reads
 *  it in the Verlauf; a banner for an hour-old fact would be noise, not news. */
