...
const TONE: Record<MeldungKind, MeldungTone> = {
  atemschutz: 'alarm', alarm: 'alarm', reminder: 'warn', gps: 'warn', review: 'info', tabLock: 'info', symbols: 'warn', basemap: 'warn', offline: 'warn', session: 'warn', update: 'calm', install: 'calm',
}
...
describe('rankMeldungen', () => {
  ...
  it('orders every kind, so the strip reads best-first from the top down', () => {
    const all: MeldungKind[] = ['install', 'update', 'session', 'offline', 'basemap', 'symbols', 'tabLock', 'review', 'gps', 'reminder', 'alarm', 'atemschutz']
    expect(rankMeldungen(all.map((k) => m(k))).map((r) => r.kind))
      .toEqual(['atemschutz', 'alarm', 'reminder', 'gps', 'review', 'tabLock', 'symbols', 'basemap', 'offline', 'session', 'update', 'install'])
  })
