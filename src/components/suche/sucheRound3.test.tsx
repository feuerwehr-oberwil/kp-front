...
import {
  addBereich, addPerson, bereichStatusOf, composerFoundLink, emptySuche, personGefunden, personKorrigiert, personView, personenViews,
  sucheChangeWords, sucheLinkLabel,
  type SucheCx,
} from '../../lib/suche'
...
import { SuchePanel } from './SuchePanel'
...
let n = 0
const cx = (at = '2026-09-25T19:10:00.000Z'): SucheCx => ({ at, newId: (p) => `${p}${++n}`, floorName: floorLabel })
const klasse = () => addPerson(emptySuche(), { name: 'Klasse 4b', count: 8, wo: 'Werkraum' }, cx())
...
describe('F7 · «Fund melden» starts from where the Trupp is, and from one', () => {
  it('the form takes the Trupp\'s place and counts from one', () => {
    const k = klasse()
    const start = addBereich(k.doc, { name: 'Dachstock', trupp: { label: 'Trupp 2', id: 't2' } }, cx())
    let last: SucheDoc = start.doc
    function Harness() {
      const [doc, setDoc] = useState(start.doc)
      const actions = useSucheActions({ suche: doc, setRaw: (d) => { setDoc(d); last = d }, canEdit: true, log: () => {}, emit: () => {}, floorName: floorLabel })
      return <SuchePanel doc={doc} floorName={floorLabel} canEdit actions={actions}
        trupps={[{ id: 't2', label: 'Trupp 2', short: 'T2' }]} uebergabe={[]}
        focus={{ fund: { truppId: 't2', bereichId: start.id! }, nonce: 1 }} onExit={() => {}} />
    }
    render(<Harness />)
    ...
    // the Trupp's place, not the group's «zuletzt gesehen» (the Werkraum)
    expect((screen.getByLabelText(C.wo) as HTMLInputElement).value).toBe('Dachstock')
    fireEvent.click(screen.getByRole('button', { name: C.submitGefunden }))
    expect(personenViews(last)[0]).toMatchObject({ found: 1, missing: 7, foundTrupp: 'Trupp 2' })
    expect(last.personen[0].log[1].bereichId).toBe(start.id)
  })
})

describe('the found place can be corrected', () => {
  it('«Korrigieren» moves the latest find, its row says so, and the «Fund» moves with it', () => {
    let d = addBereich(emptySuche(), { name: 'Zimmer 304' }, cx()).doc
    d = addBereich(d, { name: 'Zimmer 204' }, cx()).doc
    const a = addPerson(d, { name: 'Tim Muster', wo: 'Büro' }, cx())
    const [z304, z204] = a.doc.bereiche
    const g = personGefunden(a.doc, a.person.id, { bereichId: z304.id, trupp: 'Trupp 1' }, cx())
    expect(bereichStatusOf(z304, g.doc).fund).toBe(true)
    const k = personKorrigiert(g.doc, a.person.id, { name: 'Tim Muster', foundWo: 'zimmer 204' }, cx())
    expect(k.rows[0].text).toBe('Korrigiert: Tim Muster · zuletzt Büro · gefunden Zimmer 304 → Tim Muster · zuletzt Büro · gefunden Zimmer 204')
    expect(personView(k.doc.personen[0])).toMatchObject({ foundWo: 'Zimmer 204', status: 'gefunden' })
    expect(bereichStatusOf(z304, k.doc).fund).toBe(false)
    expect(bereichStatusOf(z204, k.doc).fund).toBe(true)
  })

  it('the form offers the found place once somebody was found', () => {
    const a = addPerson(emptySuche(), { name: 'Tim Muster', wo: 'Büro' }, cx())
    const g = personGefunden(a.doc, a.person.id, { wo: 'Zimmer 304' }, cx())
    let last: SucheDoc = g.doc
    function Harness() {
      const [doc, setDoc] = useState(g.doc)
      const actions = useSucheActions({ suche: doc, setRaw: (d) => { setDoc(d); last = d }, canEdit: true, log: () => {}, emit: () => {}, floorName: floorLabel })
      return <SuchePanel doc={doc} floorName={floorLabel} canEdit actions={actions}
        trupps={[]} uebergabe={[]} focus={{ personId: a.person.id, nonce: 1 }} />
    }
    render(<Harness />)
    fireEvent.click(screen.getByRole('button', { name: C.korrigierenBtn }))
    expect((screen.getByLabelText(C.korrigierenGefunden) as HTMLInputElement).value).toBe('Zimmer 304')
    fireEvent.change(screen.getByLabelText(C.korrigierenGefunden), { target: { value: 'Zimmer 305' } })
