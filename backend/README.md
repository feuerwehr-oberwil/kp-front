...
| Vehicles | the «vor Ort / verlassen» Verlauf rows and the Rapport's Fahrzeugzeiten (`reportMeta.fahrzeuge`) against `vehicle_samples`. The client's own state machine is replayed fix by fix (inside 150 m, beyond 300 m, the band between clears a pending change, 90 s to settle), so a row is expected at GPS + 90 s | a row more than `--vehicle-lag` (300 s) after the GPS, printed with the expected time, the stated and received time, and whether the writing device was online at the GPS change; late rows one device wrote in the same second are one finding (a device catching up). Also two rows for one change, a row with no GPS change behind it, and a GPS change with no row once a device had the incident open. A row whose own clock is off by more than `--clock-skew` is placed by its receipt time. An incident at 0/0 has no Einsatzort, and the section is skipped |
