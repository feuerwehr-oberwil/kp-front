...
| [`verlauf-coverage.md`](verlauf-coverage.md) | 🟢 | Which operator actions reach the **Verlauf**, which only reach the hash-chained audit/replay stream, and which reach neither. Names the deliberate silences (Zeitplan, Checklisten, Zeichnungs-Edits) and the known gaps – the Erfassungs-Poster writes no Verlauf rows at all, and the Rapportangaben have no channel. Read before assuming something is on the record. |
