...
> `/admin` disabled), and **`SEED_PIN`** (6–12 digits, not 000000/123456).
>
...
5. **Set the variables.** Four are required – the same four the compose path generates into `.env`,
   ...
   | `SEED_PIN` | 6–12 digits, not `000000`/`123456` | **refuses to boot** while `SEED_DATABASE` is on (the seed file's PIN is public) |
   ...
   railway variable set SEED_PIN=<6–12 digits> --service <app-service>
   ```
...
compose: `POSTGRES_PASSWORD` + `SECRET_KEY` (≥32 chars) + `SEED_PIN` (6–12 digits – without it
...
- **The `app` container restarts in a loop (`docker compose ps` shows it Restarting):** read the
  ...
  6–12-digit `SEED_PIN` in `.env` (not 000000/123456 – those are rejected too) and apply it:
