...
| `/api/admin/plan-alignments/*` | the pre-computed plan alignments: list (metadata only – never the building outlines), outline (one sheet’s reference geometry, drawn per review tile), preview (`?thumbnail=true`), approve (with optional corrected pairs), reject, retry, undo – nothing reaches an incident before an approval | admin |
